...
namespace pw
{
	Window::Window( const wchar_t* windowName, const ClientConfig& clientConfig ) : _oldKeyStates( ), _windowHandle( nullptr ), _mouseCursor( nullptr ), _width( clientConfig.getWidth( ) ),
		_height( clientConfig.getHeight( ) ), _bitsPerPx( clientConfig.getBitsPerPx( ) ), _windowStyle( clientConfig.getWindowStyle( ) ), _open( false ), _mouseCaptured( false ), _windowName( windowName )
	{
		if ( isOnlyInstance( ) )
		{
